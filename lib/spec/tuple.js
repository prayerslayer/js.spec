...
import { dt, valid, specize, explain1 } from '../util'
...
export class Tuple extends Spec {
  ...
  toString() {
    return `Tuple(${this.options.predicateNames.join(", ")})`
  }
  ...
  explain(path, via, value) {
    ...
    return this.options.predicates
      .map((predicate, i) => {
        ...
        if (!valid(s, v)) {
          return explain1(s, [...path, i], [...via, this.toString()], v)
        }
        return null
      })
...
export default function tuple(...predicates) {
  return new Tuple({
    predicates,
    predicateNames: predicates.map(p => p.name || p.toString())
  })
}
