...
export function toSpec(maybeSpec, checkRegistry = true) {
...
export function specize(spec, checkRegistry = true) {
  const maybeSpec = toSpec(spec, checkRegistry)
  if (!p.nil(maybeSpec)) {
    return maybeSpec
  }
  if (p.fn(spec)) {
