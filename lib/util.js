...
import { predicate as predicateSpec } from "./spec/predicate";
...
function checkNilableAndThrow(specName, predName, pred) {
  if (p.nil(pred)) {
    predName = predName ? " " + predName : "";
    throw new Error(
      `Predicate${predName} of spec ${specName} is null or undefined, probably that's not your intention.`
    );
  }
}
...
export function undefinedPredicateWarning(spec, predicates) {
  ...
  if (isIterable) {
    const iterable = Array.isArray(predicates)
      ? predicates.entries()
      : Object.entries(predicates);
    for (const [i, pred] of iterable) {
      checkNilableAndThrow(spec, i, pred);
    }
  } else {
    checkNilableAndThrow(spec, "", predicates);
  }
}
...
export function getAllKeys(obj) {
  return [
    ...
    x => [invalid, optional, count, minCount, maxCount].indexOf(x) === -1
  ); // filter own symbols
}
...
export function explain(spec, path, via, value) {
  ...
  return isRegex(s)
    ? regexExplain(s, path, via, value)
    : s.explain(path, via, value);
}
